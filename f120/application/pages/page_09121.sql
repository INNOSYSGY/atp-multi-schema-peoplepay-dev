...
wwv_flow_imp_page.create_page_item(
 p_id=>wwv_flow_imp.id(3631010077243639543)
,p_name=>'P9121_MONTHS'
,p_item_sequence=>70
...
wwv_flow_imp_page.create_page_item(
 p_id=>wwv_flow_imp.id(3631010509205639543)
...
wwv_flow_imp_page.create_page_item(
 p_id=>wwv_flow_imp.id(3631010835901639544)
...
wwv_flow_imp_page.create_page_process(
 p_id=>wwv_flow_imp.id(3631009719340639542)
...
'',
'    v_mime1:=''TIN,Employee_Number,First_Name,Last_Name,Other_Names,Address,Pay_Frequency,Period_Employed,Employee_Type,Value_7A,Value_7B,Value_7C_Taxable,Value_7C_Non_Taxable,Total_Income,Personal_Allowance,Employee_NIS_Contribution,Medical_Life_Insu'
||'rance_Premiums_Deduction,Total_Deductions,Tax_Deducted,Date_Of_Birth''||chr(13)||chr(10);',
...
'            max(EMPLOYEE_TYPE)||'',''||             ',
'            sum(nvl(round(Value_7A),0)) ||'',''|| ',
'            sum(nvl(round(Value_7B),0)) ||'',''|| ',
'            sum(nvl(round(Value_7C_Taxable),0)) ||'',''|| ',
'            sum(nvl(round(Value_7C_Non_Taxable),0)) ||'',''|| ',
'            sum(nvl(round(Total_Income),0)) ||'',''|| ',
'            sum(nvl(round(TAX_FREE),0)) ||'',''|| ',
'            sum(nvl( round(NIS_EMPLOYEE),0)) ||'',''|| ',
'            sum(nvl( round(Value_Medical),0)) ||'',''|| ',
'            sum(nvl(round(Total_Deduction) ,0)) ||'',''|| ',
'            sum(nvl(round(Tax_Deducted),0)) ||'',''|| 			',
...
'          '',''|| ',
'          nvl(round(Total_Value_7A),0)||'',''|| ',
'          nvl(round(Total_Value_7B),0)||'',''|| ',
'          nvl(round(TOTAL_VALUE_7C_TAXABLE),0)||'',''|| ',
'          nvl(round(TOTAL_VALUE_7C_NON_TAXABLE) ,0)||'',''|| ',
'          nvl(round(TOTAL_TOTAL_INCOME),0)||'',''|| ',
'          nvl(round(Total_Statutory_Deduction) ,0)||'',''|| ',
'          nvl(round(Total_NIS_Contribution),0)||'',''|| ',
'          nvl(round(TOTAL_VALUE_MEDICAL),0)||'',''||',
'          nvl(round(Total_Deduction),0) ||'',''|| ',
'          nvl(round(Total_Tax_Deducted) ,0)||'',''|| ',
'          nvl(NULL,''''), 1 ind',
'          ',
'FROM GRA_FORM2_HD',
