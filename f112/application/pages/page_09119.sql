...
wwv_flow_imp_page.create_page_item(
 p_id=>wwv_flow_imp.id(3325006539401971074)
...
wwv_flow_imp_page.create_page_item(
 p_id=>wwv_flow_imp.id(3325006907018971075)
...
wwv_flow_imp_page.create_page_process(
 p_id=>wwv_flow_imp.id(3325006072110971073)
...
'              where o.ORGANISATION_TYPE=''BANK'' and UPPER(trim(o.ORGANISATION_NAME))=upper(trim(bank_branch_id_hist)) and o.USER_ORG_ID=a.org_id',
'             )          ',
'             ',
'             ||''|''||replace(a.bank_account_no_hist,''-'','''')||''|''||c.BANK_ACCOUNT_TYPE||''|''||a.first_name_hist||'' ''||a.surname_hist||''|''|| TRIM(TO_CHAR(a.net_pay, ''999999999.99''))||''|''||''Payroll;'' new_format',
'                from pa_pmg_payrolldtl a',
'             join HR_RCM_EMPLOYEE b on a.EMP_ID=b.id',
'             join HR_RCM_INDIVIDUAL c on b.IND_ID=c.id',
'       ',
'             where upper(trim(bank_branch_id_hist)) in',
...
'             and length(trim( replace(bank_account_no_hist,''-'','''')))<=20',
